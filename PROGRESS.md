...
- [ ] **Plan 5b — Scénario en direct** : curseurs/menus/cases sur ⑦ (20 champs), recalcul live ①④⑤⑥⑦⑧, score/100 automatique (rendement net-net/cash-on-cash/TRI, sans pondération utilisateur), autosave débouncée ~600ms, 2/3 dettes du Plan 5a corrigées. **Implémenté, poussé sur `main` le 2026-07-25, en attente de validation Vercel.**
- [ ] **Plan 5c — Photos & Documents** : upload glisser-déposer avec compression image transparente (`browser-image-compression`), grille photos réorganisable (`@dnd-kit/sortable`) + légende + lightbox plein écran, liste documents typés + téléchargement, limites 8 Mo/20 photos/10 documents calculées contre le quota gratuit Supabase. **Implémenté, poussé sur `main` le 2026-07-26, en attente de validation Vercel.**
...
- **Scénario en direct (Plan 5b)** : `src/lib/calc/metrics.ts` (`computeInvestmentMetrics` — orchestration unique financement/cashflow/fiscalité/TRI, **source unique** consommée par `SectionVerdict` ET `SectionCalculs`, ne pas redériver) · `src/lib/calc/scoring.ts` (`computeScoreSur100`, `computeVerdictFromScore` — score/100 fixe 40/35/25, sans pondération utilisateur, profils = Plan 7) · `src/components/app/fiche/FicheScenarioSections.tsx` (wrapper client, seul point de bascule, porte le state `scenario` + statut autosave, rend ①②③④⑤⑥⑦⑧ dans cet ordre — `page.tsx` ne rend plus que ce wrapper + ⑨) · `src/components/app/fiche/scenario-controls.tsx` (`SliderControl`/`SelectControl`/`CheckboxControl`/`NumberControl`, génériques et réutilisables) · `src/lib/hooks/use-debounced-scenario-save.ts` (`useDebouncedScenarioSave`, debounce 600ms) · server action `updatePropertyScenario` (`src/app/(app)/app/p/[projectId]/bien/[propertyId]/actions.ts`, clamp défensif). `computeInFineMonthlyPayment` (`financing.ts`) et `REGIME_LABELS` (`tax.ts`) sont maintenant les sources uniques (dettes #1/#2 du Plan 5a corrigées).
- **Dette technique restante (non bloquante)** : base d'intérêt différente entre ⑤ Calculs (tableau d'amortissement réel) et ⑥ Fiscalité (capital initial × taux, volontairement laissé en l'état au Plan 5b) — à unifier quand les formules fiscales exactes arriveront. Nouveaux points notés en revue finale Plan 5b (mineurs, non bloquants) : `updated_at` non avancé par l'autosave (à corriger avant que la Phase 3 surveillance en dépende) · `works_provision`/`gli`/`pno` éditables mais ne nourrissent aucun calcul (hérité du Plan 5a).
- **Photos & Documents (Plan 5c)** : `src/lib/file-limits.ts` (constantes/limites + `isAcceptedPhotoType`/`isAcceptedDocumentType`/`DOC_TYPE_OPTIONS` — source unique consommée côté client ET serveur) · `src/lib/hooks/use-file-upload.ts` (`useFileUpload`, compression transparente via `browser-image-compression`) · `src/lib/hooks/use-confirm-delete.ts` (`useConfirmDelete`, confirmation à deux clics partagée) · `src/components/app/fiche/{FileDropzone,PhotoLightbox,PhotoGrid,DocumentList}.tsx` · server actions photos/documents (upload/suppression/réorganisation/légende/type) ajoutées à `actions.ts` (`src/app/(app)/app/p/[projectId]/bien/[propertyId]/`) · `page.tsx` génère les URLs signées (1h) pour chaque photo/document existant. Réorganisation des photos = même pattern de fractional indexing que le board Kanban (`computeDropPosition`). Dette : `updated_at` non plus avancé ici non plus (même limite que 5b) ; suppression/réorganisation/légende en échec réseau restent silencieuses (`.catch(() => {})`) ; `URL.createObjectURL` des previews jamais révoquées (impact négligeable vu les plafonds 20/10).
...
### Session 16 — 2026-07-26 — Plan 5c exécuté (photos & documents)
**Étape : 🔨 impl — Plan 5c**
- **Brainstorm + spec** : `docs/superpowers/specs/2026-07-26-plan5c-photos-documents-design.md`. Décisions clés de cadrage : version « riche » (glisser-déposer + réorganisation + légende + lightbox + type de document), limites 8 Mo/fichier + 20 photos/10 documents par bien calculées explicitement contre le quota gratuit Supabase (**1 Go par projet, partagé entre tous les utilisateurs confondus, pas par utilisateur** — vérifié dans la doc Supabase en session), compression d'image automatique et transparente (`browser-image-compression`, nouvelle dépendance validée comme `@dnd-kit`). Digression notée hors scope : alternative de stockage moins chère à l'échelle (Cloudflare R2) — à évaluer avant la Phase 6, pas avant (mémoire `infra-stockage-scaling-futur`).
- **Plan écrit** : `docs/superpowers/plans/2026-07-26-plan5c-photos-documents.md` (12 tâches, code complet).
- **Exécuté en subagent-driven-development** (un sous-agent implémenteur + un reviewer par tâche), directement sur `main`. **11/12 tâches approuvées sans round de correction** ; une revue (Task 3, dépendance npm) a flaggé `^2.0.2` comme non conforme à une exigence de « version exacte pinnée » — adjudiqué comme un faux positif : cette exigence venait d'une formulation ajoutée par le contrôleur dans le prompt de dispatch, pas du plan réel, et `^2.0.2` est cohérent avec la convention caret déjà utilisée par toutes les autres dépendances du repo (`@dnd-kit/*`, `@supabase/*`, `gsap`).
- **Livré** : `file-limits.ts`, types `PropertyDetailPhotoWithUrl`/`DocumentWithUrl`, hook `useFileUpload` (compression), Server Actions photos (upload/suppression/réorganisation/légende) et documents (upload/suppression/type), `FileDropzone`/`PhotoLightbox`/`PhotoGrid`/`DocumentList`, `SectionHumain` réécrite (placeholders → composants réels), `page.tsx` génère les URLs signées.
- **Vérification finale** : `npx tsc --noEmit` 0 erreur, `npm run build` ✅, `npm run lint` ✅ (0 erreur, 5 warnings préexistants hors périmètre). QA manuelle **partielle** (même limite qu'au Plan 5b) : smoke test `curl` confirmé (rendu 200, dropzones photos/documents présentes, aucune erreur serveur) — **l'interactivité réelle (drag-drop upload → compression → apparition immédiate → réorganisation → lightbox → suppression) n'a pas pu être testée dans un vrai navigateur** (pas de `chromium-cli` disponible) : à confirmer par l'utilisateur sur Vercel.
- **Revue finale de branche** : Ready to merge (With fixes — appliqués). 0 Critical. **2 Important corrigés immédiatement** (commit `383e14d`) : (1) confirmation de suppression dédoublonnée dans un nouveau hook partagé `useConfirmDelete` (PhotoGrid + DocumentList dupliquaient la même logique) ; (2) légende photo trimmée côté client pour s'aligner avec le trim déjà fait côté serveur (évitait une divergence transitoire sur une légende « espaces seuls »). 4 points mineurs tracés (voir Repères techniques ci-dessus), non bloquants.
- Commits `2a57472`→`383e14d` (+ spec/plan). **Poussé sur `main`** (accord utilisateur).
- **Prochaine action** : validation utilisateur sur Vercel des Plans 5b **et** 5c (aucun des deux n'a encore reçu de « c'est bon ») → cocher les deux → brainstorm Plan 6 (vue Carte) ou Plan 7 (Comparer/Arbitrage), au choix de l'utilisateur.

### Session 15 — 2026-07-25 — Plan 5b exécuté (scénario en direct)
**Étape : 🔨 impl — Plan 5b**
- **Brainstorm + spec** : `docs/superpowers/specs/2026-07-24-plan5b-scenario-live-design.md`. Décision clé de cadrage : le Verdict (score) doit lui aussi réagir au scénario (demande utilisateur en cours de brainstorm) — arbitré en score/100 **automatique et fixe** (rendement net-net 40% / cash-on-cash 35% / TRI 25%), **sans** pondération utilisateur ni profils de priorité (ça reste le Plan 7). Correction d'architecture actée dans le plan (pas dans la spec) : ②③ rendues à l'intérieur du wrapper client pour préserver l'ordre visuel ①→⑧.
- **Plan écrit** : `docs/superpowers/plans/2026-07-24-plan5b-scenario-live.md` (16 tâches, code complet).
- **Exécuté en subagent-driven-development** (un sous-agent implémenteur + un reviewer par tâche), directement sur `main`. **Blocage environnement en cours de route** : tous les dispatchs d'agents échouaient (`tools.X.custom.input_schema does not support oneOf...`) à cause d'un serveur MCP au schéma invalide — résolu en désactivant les serveurs Higgfields/OpenLegi/firecrawl via `/mcp` (non utilisés pour ce chantier), aucun lien avec le code du projet.
- **15/15 tâches approuvées** sans aucun round de correction (spec compliance + qualité clean à chaque revue) : `calc/scoring.ts` (score/100), `financing.ts`/`tax.ts` (dettes techniques #1/#2 du Plan 5a corrigées), `calc/metrics.ts` (orchestration unique, nouvelle décomposition non prévue dans la spec mais nécessaire pour que ① et ⑤ ne divergent jamais), server action + hook autosave débouncée, `SectionVerdict`/`SectionCalculs` réécrits, `SectionFiscalite`/`SectionCharges` passés en `use client`, `SectionFinancement` dédoublonné, contrôles génériques + `SectionScenario` interactive (20 champs), wrapper `FicheScenarioSections`, intégration `page.tsx`.
- **Vérification finale** : `npx tsc --noEmit` 0 erreur, `npm run build` ✅, `npm run lint` ✅ (0 erreur, 5 warnings préexistants hors périmètre). QA manuelle **partielle** : smoke test via `curl` sur un bien seedé réel (rendu 200, 9 sliders présents, toutes les sections ①→⑧ rendues, valeurs par défaut du scénario visibles, aucune erreur/404 déclenché) — **l'interactivité live (drag slider → recalcul → autosave → persistance) n'a pas pu être testée dans un vrai navigateur** (pas de `chromium-cli` disponible dans cet environnement Windows) : à confirmer par l'utilisateur sur Vercel.
- **Revue finale de branche** : Ready to merge (Yes), 0 Critical/Important. 4 points mineurs tracés (voir Repères techniques ci-dessus) : `updated_at` non avancé par l'autosave, type de patch un peu trop large (`Partial<PropertyScenarioRow>` vs `ScenarioPatch`), champs € sans borne haute côté client (borne serveur = conforme au plan), `works_provision`/`gli`/`pno` toujours sans effet sur les calculs (hérité).
- Commits `92ee0ce`→`8181704` (+ spec `ce213bc`, plan `237e9f1`). **Poussé sur `main`** (accord utilisateur).
- **Prochaine action** : validation utilisateur sur Vercel → « c'est bon » → cocher Plan 5b → brainstorm Plan 5c (photos/documents) ou Plan 6 (vue Carte), au choix de l'utilisateur.
