...
- **Dette technique connue (non bloquante, notée en revue)** : la formule de mensualité "in fine = intérêts seuls" est dupliquée dans `SectionFinancement.tsx` et `SectionCalculs.tsx` (pourrait devenir une fonction partagée dans `financing.ts`) · les libellés français des régimes fiscaux existent en 2 copies (`tax.ts:REGIME_LABELS` et `SectionScenario.tsx:TAX_REGIME_LABELS`) — à surveiller si un 3e usage apparaît.
