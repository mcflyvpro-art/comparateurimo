 ...
 * section, pas de dots — seulement les scènes (image → vidéo), un léger voile
 * flouté devant, et l'indice "scroll pour entrer".
 *
 * Pin contenu (pas fixed) : `container` porte la hauteur totale du scroll et
 * `pin` (sticky, top:0, 100vh) reste épinglé tant que `container` traverse le
 * viewport, puis se libère naturellement une fois son bord bas atteint — le
 * fond ne reste jamais plaqué sur le reste de la page après la dernière scène.
 */
...
export type ScrollWorldConfig = {
  ...
  crossfade?: number;
  atmosphere?: boolean;
  /** Rayon (px) du voile flouté devant la vidéo — masque la compression/le grain. */
  blur?: number;
...
export function mountScrollWorld(container: HTMLDivElement, config: ScrollWorldConfig): () => void {
  ...
  const BLUR = config.blur != null ? config.blur : 5;
  ...
  // `pin` reste sticky tant que `container` (hauteur totale du scroll) traverse
  // le viewport, puis se libère naturellement — jamais plaqué sur la suite.
  ...
  const veil = el("div", "sw-veil");
  veil.style.setProperty("--sw-blur", BLUR + "px");
  ...
  [sky, stage, veil, hint].forEach((n) => pin.appendChild(n));
  ...
  let stageX = 0;
  let totalW = 0;
  let containerH = 0; // hauteur totale de `container`, en px
  let ticking = false;
  ...
  function layout() {
    ...
    totalW = off;
    containerH = totalW * vh + vh; // +1vh de marge pour que le dernier vol se termine
  ...
  // Progression locale dans la section : 0 tant que `container` n'a pas
  // atteint le haut du viewport, avance avec le scroll jusqu'à ce que le bas
  // de `container` atteigne le haut du viewport — fin du pin, libération.
  function read() {
    const rectTop = container.getBoundingClientRect().top;
    const y = clamp(-rectTop, 0, Math.max(0, containerH - vh));
    ...
    if (cur.accent) pin.style.setProperty("--sw-accent", cur.accent);
...
function injectCSS() {
  ...
  const css = `
  .sw-root{position:relative;--sw-bg:#0A0A0B;--sw-ink:#F4F4F4;--sw-ink-soft:#9F9EA0;--sw-accent:#545164;color:var(--sw-ink);overflow:hidden;}
  .sw-pin{position:sticky;top:0;height:100svh;overflow:hidden;background:var(--sw-bg);}
  ...
  .sw-veil{position:absolute;inset:0;z-index:15;pointer-events:none;backdrop-filter:blur(var(--sw-blur,5px)) saturate(1.05);-webkit-backdrop-filter:blur(var(--sw-blur,5px)) saturate(1.05);background:color-mix(in srgb,var(--sw-bg) 10%,transparent);}
